"""
Training script for Nanda et al. (2023) modular addition with AGOP and Lazy-Rich tracking

This script trains models on modular addition while tracking:
...
Supports both MLP and ReLU Transformer architectures for comparison.
...
def train_with_tracking(
    ...
    for epoch in tqdm(range(n_epochs), desc="Training"):
        ...
        if epoch % log_freq == 0 or epoch == n_epochs - 1:
            ...
            # Compute metrics at agop_freq intervals
...
def main():
    parser = argparse.ArgumentParser(description='Train Nanda model with AGOP and Lazy-Rich tracking')
    ...
    if args.experiment_name is None:
        args.experiment_name = f'nanda_{args.architecture}_{args.optimizer}_wd{args.weight_decay}_seed{args.seed}'
    
    print("="*80)
    print("Nanda Modular Addition with AGOP + Lazy-Rich Tracking")
    print("="*80)
