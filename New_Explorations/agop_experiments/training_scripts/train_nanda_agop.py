"""
Training script for Nanda et al. (2023) modular addition with Input-Gradient AGOP tracking

This script trains the one-layer ReLU transformer on modular addition while tracking
tractable input-gradient AGOP metrics to analyze grokking mechanisms.

Key features:
- Tracks input-gradient AGOP (tractable: 2*p × 2*p for modulus p)
- Computes eigengap, spectral radius, trace, variation collapse ratio
- Saves comprehensive metrics for analysis
- Supports multiple optimizers (AdamW, Muon, SGD)
"""
...
def main():
    parser = argparse.ArgumentParser(description='Train Nanda model with Input-Gradient AGOP tracking')
    ...
    parser.add_argument('--save_dir', type=str, default='./results/agop_experiments/nanda')
    ...
    if args.experiment_name is None:
        args.experiment_name = f'nanda_{args.architecture}_{args.optimizer}_p{args.p}_lr{args.lr}_wd{args.weight_decay}_seed{args.seed}'
    
    print("="*80)
    print("Nanda Modular Addition with Tractable Input-Gradient AGOP (One-Hot)")
    print("="*80)
    ...
    if args.architecture == 'mlp':
        ...
        model = OneHotReLUTransformer(p=args.p, d_model=args.d_model, 
                                      n_heads=args.n_heads, d_mlp=args.d_mlp).to(device)
        print(f"Model: ReLU Transformer with {sum(p.numel() for p in model.parameters()):,} parameters")
    else:
