...
def main():
    ...
    # Loss function (Paper 5 specific)
    parser.add_argument('--use_mse_loss', action='store_true', default=True,
                       help='Use MSE loss (as in Omnigrok paper) instead of CrossEntropy')
    ...
    # Set optimizer-specific learning rate if needed
    if args.optimizer in ['muon', 'muonw'] and args.lr == 0.001:
        # Override default Muon LR to official value
        args.lr = 0.02
        print(f"Note: Using Muon default lr=0.02 (not 0.001)")
    ...
    # Use float64 as in Omnigrok paper
    ...
    print(f"  Loss function: {'MSE (one-hot)' if args.use_mse_loss else 'CrossEntropy'}")
    print(f"  Dtype: float64 (as in paper)")
