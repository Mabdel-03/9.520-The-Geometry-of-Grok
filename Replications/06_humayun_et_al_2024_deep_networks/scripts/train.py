...
from adversarial_utils import evaluate_multiple_epsilons
...
def get_dataset(dataset_name, train_size=None, data_dir='./data'):
    ...
    Args:
        dataset_name: 'mnist', 'cifar10', 'cifar100', 'imagenette'
    ...
    if dataset_name == 'mnist':
        ...
        train_dataset = torchvision.datasets.ImageFolder(
            root=imagenette_dir / 'train',
            transform=transform_train
        )
        test_dataset = torchvision.datasets.ImageFolder(
            root=imagenette_dir / 'val',
...
def train_model(
    ...
    Args:
        ...
        adv_epsilons: List of epsilon values for PGD attack (default: [0.06, 0.10, 0.13, 0.16, 0.20])
        adv_eval_batches: Number of batches to use for adversarial evaluation (speed vs accuracy tradeoff)
    """
    # Set random seed
    ...
    # Setup adversarial testing if enabled
    if adv_epsilons is None:
        adv_epsilons = [0.06, 0.10, 0.13, 0.16, 0.20]
    ...
    # Add adversarial accuracy fields if enabled
    if enable_adversarial:
        for eps in adv_epsilons:
            history[f'adv_acc_eps_{eps:.2f}'] = []
        print(f"Adversarial testing enabled with epsilons: {adv_epsilons}")
        print(f"Using {adv_eval_batches} batches for adversarial evaluation")
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % log_interval == 0 or epoch == n_epochs - 1:
            ...
            # Evaluate adversarial robustness if enabled
            adv_results = {}
            if enable_adversarial:
                adv_results = evaluate_multiple_epsilons(
                    model, test_loader, adv_epsilons, 
                    device=device, max_batches=adv_eval_batches
                )
            ...
            # Log adversarial accuracies
            if enable_adversarial:
                for eps in adv_epsilons:
                    history[f'adv_acc_eps_{eps:.2f}'].append(adv_results[eps])
            
            # Print progress
            log_str = (f"Epoch {epoch:5d} | Train Loss: {train_loss:.4f} | "
                      f"Train Acc: {train_acc:.4f} | Test Loss: {test_loss:.4f} | "
                      f"Test Acc: {test_acc:.4f}")
            
            if enable_adversarial:
                adv_str = " | Adv Acc: " + ", ".join([f"ε={eps:.2f}:{adv_results[eps]:.4f}" 
                                                       for eps in adv_epsilons])
                log_str += adv_str
...
def main():
    ...
    parser.add_argument('--enable_adversarial', action='store_true', 
                       help='Enable adversarial robustness testing')
    parser.add_argument('--adv_epsilons', type=float, nargs='+', default=None,
                       help='Epsilon values for PGD attack (default: [0.06, 0.10, 0.13, 0.16, 0.20])')
    parser.add_argument('--adv_eval_batches', type=int, default=20,
                       help='Number of batches for adversarial evaluation')
    ...
    train_model(
        ...
        adv_epsilons=args.adv_epsilons,
        adv_eval_batches=args.adv_eval_batches
    )
