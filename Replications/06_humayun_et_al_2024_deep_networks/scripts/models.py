...
class ResNet18NoBN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        # Adaptive pooling to handle different input sizes (CIFAR: 32x32, Imagenette: 224x224)
...
def get_model(model_name, dataset='mnist', **kwargs):
    ...
    Args:
        ...
        dataset: 'mnist', 'cifar10', 'cifar100', 'imagenette'
    """
    if model_name == 'mlp':
        if dataset == 'mnist':
        ...
        num_classes = 10 if dataset in ['mnist', 'cifar10', 'imagenette'] else 100
